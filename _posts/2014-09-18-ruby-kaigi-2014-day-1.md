...
commentary (it's like [Rails weekly](https://rails-weekly.goodbits.io) but for
Ruby trunk), gave a behind-the-scene view of CRuby development.

There are 84 registered accounts on SVN (including a [bot](LINKME)), 50 of which
who had at least one commit since Ruby Kaigi 2013. @nogachika introduced some of
them in his talk, some highlights:
...
matter worse, the non-deterministic nature of parallel execution make these bugs
...
As I mentioned earlier, this is one of the key driver for the Rails team to
...
The pitfall associated with this approach is that the execution time of your
program, you could accumulate many symbols that you no longer need, thus
leaking memory. This is particularly problematic for programs that has a long
...
Transactional memory is similar to how a database work. It allows the programmer
to place a block of code inside a memory transaction, during which all the
memory operations will be perceived as a single atomic operation from other
threads (or fails and the operations are rolledback and aren't observable from
outside of the thread that requested the transaction).
